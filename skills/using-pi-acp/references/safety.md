...
Source: `src/danger.js`, enforced in `src/client.js`'s permission handler.

Every omp tool call arrives as an ACP `session/request_permission` whose
`toolCall.rawInput.command` holds the shell command (for `kind: "execute"`). Before
approving, the command is matched against destructive patterns. A match is
**rejected** (the client selects a `reject_once` option and logs
`BLOCKED dangerous command (<why>): <cmd>`); everything else is approved.
...
`pi-acp stop` is deliberately cautious:
...
3. **Never touches the session's working directory** or any file the agent
   created/edited there — pi-acp only removes its own runtime state.
4. **Confirms interactively** in a TTY; auto-proceeds when scripted (no TTY) or
   with `-y`, so agent callers never hang.

Shutdown order: ACP `session/close` → remove runtime files → exit. `SIGTERM`/
